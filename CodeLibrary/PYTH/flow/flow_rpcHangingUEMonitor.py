...
def judging_and_output(csv_file="", number=0):
    ...
    file_path = os.path.join(os.path.dirname(csv_file), "report_rpcHangingUE_" + str(number) + ".csv")
    if new_data:
...
def log_collection(file_ue_list="", csv_file="", log_file="", number=0):
    ...
    if site_list:
        ...
        file_name = "ue_list_" + str(number) + "_" + config.date_time
...
if __name__ == '__main__':
    ...
    # log_path = os.path.join(os.path.join(config.tool_path, "log.sample"), "flow_rpcHangingUEMonitor")
