...
def ng_delta_time_summary(data_in, debug=False):
    ...
    for row in data_in:
        ...
        data_row = [source, round(phase_a, 3), round(phase_b, 3), round(phase_c, 3), round(phase_d, 3),
                    round(phase_e, 3), round(phase_a + phase_b + phase_c, 3), round(phase_d + phase_e, 3), target]
...
def ng_handover_parsing(file_path, debug=False):
    ...
    if debug:
        write_data_to_csv(ho_source, os.path.join(file_path, "ng_ho_source.csv"))
    ...
    if debug:
        write_data_to_csv(ho_matching, os.path.join(file_path, "ng_ho_matching.csv"))
...
def xn_delta_time_summary(data_in, debug=False):
    head = [["ue_source", "PhaseA", "PhaseB", "PhaseC", "PhaseD", "PhaseE", "TotalSrc(A+C)", "TotalTar(B+D+E)",
             "ue_target"]]
    data = []
    for row in data_in:
        s = row[3:]
        ...
        data_row = [source, round(phase_a, 3), round(phase_b, 3), round(phase_c, 3), round(phase_d, 3),
                    round(phase_e, 3), round(phase_a + phase_c, 3), round(phase_b + phase_d + phase_e, 3), target]
        data.append(data_row)
    if data:
        head.extend(data)
    else:
        ...
        for file_name in files:
            ...
            if extension in ".flow" or extension in ".flw":
                ...
                if temp_data:
                    ho_target.extend(temp_data)
    if debug:
        write_data_to_csv(ho_source, os.path.join(file_path, "xn_ho_source.csv"))
        write_data_to_csv(ho_target, os.path.join(file_path, "xn_ho_target.csv"))

    # ho matching between source and target
    ho_matching = xn_source_matching_target(ho_source, ho_target)
    if debug:
        write_data_to_csv(ho_matching, os.path.join(file_path, "xn_ho_matching.csv"))
